...
`html_jsonl_slicer.py` 将 Confluence 风格 HTML 手册按 TOC 切分为单个 JSONL 文件。脚本调用 `html_extractor.py` 提供的 TOC 解析和正文抽取库函数，并统一走 JSONL 写入与校验流程。
...
| `select_chapters(chapters, from_n, to_n)` | 按 TOC 的 1-based `index` 范围筛选章节 | TOC entries、起止 index | TOC entries |
| `build_entry(chapter, content)` | 将 TOC 章节和抽取内容整理为 JSONL entry | 章节 dict、content list | entry dict |
...
| `title` | TOC 标题 |
| `depth` | TOC 层级 |
| `source_file` | 原 HTML 文件名，由 TOC entry 的 `file` 字段映射而来 |
| `anchor` | 可选；TOC href 中存在 anchor 时保留 |
| `content` | 由 `html_extractor.extract_chapter_content()` 生成的 `text/table/code` 内容块数组 |

## 关键边界条件

- `html_extractor.parse_toc()` 会去重相同 `(file, anchor)`，并保留原始 index。
