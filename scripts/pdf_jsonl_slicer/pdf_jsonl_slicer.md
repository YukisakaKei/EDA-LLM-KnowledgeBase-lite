...
PDF 切片脚本的 JSONL 版本。它复用旧 `scripts/pdf_slicer.py` 中已经验证过的 PDF 书签解析、页码/坐标切片、正文/表格提取、页眉页脚剔除、跨页合并、OCR 和 PyMuPDF 文本后端，只把输出层改为单个 `.jsonl` 文件。
...
| `extract_content_for_chapter()` | 调用旧 PDF slicer 的提取函数，返回该章节的 `content` 数组 | 章节、模式、后端 | content list |
...
- 默认页码范围模式沿用旧 `pdf_slicer.py` 行为，可能与相邻章节共享边界页。
