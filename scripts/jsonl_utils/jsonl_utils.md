...
公共 JSONL 工具模块，供 PDF/HTML 等 slicer 复用。它只处理 JSONL 中间格式的字段规范、内容块规范、校验和写入，不负责从 PDF 或 HTML 提取内容。

## 核心函数

| 函数 | 作用 | 输入 | 输出 |
|---|---|---|---|
| `normalize_entry(entry)` | 规范化单条记录，将旧字段 `file` 映射为 `source_file`，并规范化 `content` | dict-like entry | 新 dict |
...
- `source_file` 可选；如果输入存在旧字段 `file` 且没有 `source_file`，会自动映射。
- `text` 和 `code` 块必须包含 `lines: list[str]`。如果旧块只有 `text` 字符串，会按换行拆成 `lines`。
