---
source: knowledge/Innovus/cui/jsonl/DBcom__211.jsonl | entries: [0036, 0038, 0042]
---
...
- **路径**：`knowledge/Innovus/cui/jsonl/DBcom__211.jsonl`
- **内容**：包含所有 CUI 对象类型的完整定义、属性列表、类型说明和默认值
- **查询方式**：按对象类型名称或 entry index 检索，例如查询 `inst` 对象可定位到 entry index=0036

### Legacy 数据结构

查询 Legacy 的具体数据结构定义和属性说明，请参考：
- **路径**：`knowledge/Innovus/legacy/jsonl/dbSchema__211.jsonl`
- **内容**：包含所有 Legacy 对象类型的完整定义、属性列表、类型说明和默认值
- **查询方式**：按对象类型名称或 entry index 检索
